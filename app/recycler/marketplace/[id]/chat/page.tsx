...
export default function RecyclerListingChatPage({
  params,
}: {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (result.error) {
        ...
        setError('Chat belum tersedia. Bid Anda harus diterima terlebih dahulu oleh company untuk memulai percakapan.')
      }
      setLoading(false)
    }
    load()
...
<AlertTriangle className="w-9 h-9 text-amber-700" strokeWidth={1.8} />
          ...
          <p className="text-xs text-muted mb-8 leading-relaxed">
            {error?.includes('belum tersedia')
              ? 'Pastikan bid Anda telah diterima oleh company untuk membuka chat.'
              : 'Pastikan listing memiliki bid yang sudah diterima.'}
          </p>
          <Link
            href={`/recycler/marketplace/${listingId}`}
  ...
  return (
    <div className="h-[100dvh] flex flex-col bg-surface">
      ...
      <div className="bg-surface border-b border-border px-4 sm:px-6 py-3 flex items-center gap-3 flex-shrink-0 shadow-[0_2px_8px_-4px_rgba(11,31,22,0.06)]">
        ...
        <div className="w-10 h-10 rounded-full bg-sage/20 text-sage-dark flex items-center justify-center flex-shrink-0 text-sm font-extrabold ring-4 ring-sage/10">
          {(thread.companies?.name || 'C').charAt(0).toUpperCase()}
        </div>

        <div className="flex-1 min-w-0">
          <h1 className="text-sm font-extrabold text-ink tracking-tight truncate">
            {thread.companies?.name || 'Company'}
          </h1>
