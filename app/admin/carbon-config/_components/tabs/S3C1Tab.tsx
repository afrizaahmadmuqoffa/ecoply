...
import {
  createS3C1AverageFactor,
  deactivateS3C1AverageFactor,
  createS3C1SpendFactor,
  deactivateS3C1SpendFactor,
...
export default function S3C1Tab({ avgFactors, spendFactors }: Props) {
...
function AverageSection({ rows }: { rows: AvgFactorRow[] }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result = await createS3C1AverageFactor({
    ...
    if (result.error) { setError(result.error); toast.error(result.error) }
      ...
      toast.success('Faktor Scope 3 Cat 1 berhasil ditambahkan')
      setSuccess(true)
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-5 items-start">
      <div className="lg:col-span-2 lg:sticky lg:top-6">
        <div className="bg-surface border border-border rounded-[18px] p-6 shadow-[0_12px_24px_-16px_rgba(11,31,22,0.06)]">
          <h3 className="text-lg font-extrabold text-ink tracking-tight mb-1">
            Material <span className="text-sage">Average.</span>
          </h3>
          <p className="text-xs text-muted mb-4 leading-relaxed">
            Metode Average-Data / Physical-Based.
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div>
              ...
              <input required placeholder="e.g. Steel - BOF" value={form.material_name} onChange={(e) => s('material_name', e.target.value)}
      ...
      <div className="lg:col-span-3">
        <MasterTable rows={rows} onDeactivate={deactivateS3C1AverageFactor}
          editTable="s3c1_average_factors" editTitle="Average Material Factor"
          editFields={[
          ...
          emptyText="Belum ada faktor material."
        />
      </div>
    </div>
  )
...
function SpendSection({ rows }: { rows: SpendFactorRow[] }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault(); setError(null); setLoading(true)
    const result = await createS3C1SpendFactor({
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-5 items-start">
      <div className="lg:col-span-2 lg:sticky lg:top-6">
        <div className="bg-surface border border-border rounded-[18px] p-6 shadow-[0_12px_24px_-16px_rgba(11,31,22,0.06)]">
          <h3 className="text-lg font-extrabold text-ink tracking-tight mb-1">
            Spend <span className="text-sage">Based.</span>
          </h3>
          <p className="text-xs text-muted mb-5 leading-relaxed">
            Metode EEIO / Spend-Based.
          </p>
          <form onSubmit={handleSubmit} className="space-y-3">
            <div>
              <label className="block text-[10px] font-bold tracking-[0.14em] uppercase text-ink mb-1.5">
              ...
              <input required placeholder="e.g. Iron and Steel" value={form.sector_name} onChange={(e) => s('sector_name', e.target.value)}
                className="w-full px-4 py-2.5 bg-canvas border border-border rounded-xl text-sm text-ink placeholder:text-muted/60 focus:outline-none focus:border-sage focus:ring-2 focus:ring-sage/20 transition-all" />
            </div>
            ...
            <div>
              ...
              <input required type="text" inputMode="decimal" placeholder="e.g. 0.00000045 (untuk IDR)" value={form.ef_kg_co2e} onChange={(e) => s('ef_kg_co2e', e.target.value)}
                className="w-full px-4 py-2.5 bg-canvas border border-border rounded-xl text-sm text-ink font-mono focus:outline-none focus:border-sage focus:ring-2 focus:ring-sage/20 transition-all" />
              <p className="text-[10px] text-muted mt-1.5 flex items-center gap-1.5">
                ...
                Angka sangat kecil untuk IDR — gunakan notasi desimal penuh
              </p>
            </div>

            <div>
              <label className="block text-[10px] font-semibold text-ink mb-1">Database EEIO</label>
              <input placeholder="e.g. US EEIO 2.0, DEFRA" value={form.eeio_database} onChange={(e) => s('eeio_database', e.target.value)}
      ...
      <div className="lg:col-span-3">
        <MasterTable rows={rows} onDeactivate={deactivateS3C1SpendFactor}
          editTable="s3c1_spend_factors" editTitle="Spend-Based Sector Factor"
          editFields={[
          ...
          emptyText="Belum ada faktor spend-based."
        />
      </div>
    </div>
  )
