...
import { updateS3C2 } from '@/lib/supabase/actions/carbon-activity'
...
const METHOD_INFO = {
  ...
  average_data: {
    label: 'Average Data',
    hint: 'Faktor rata-rata per jenis aset kapital',
    icon: (
...
export default function EditS3C2Form({ id, avgFactors, spendFactors, initial, onSaved }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaved(true)
    toast.success('Draft Scope 3 Cat 2 berhasil diperbarui')
    router.refresh()
    ...
    <div className="bg-gradient-to-br from-mint/40 via-sage/5 to-surface border border-sage/20 rounded-[18px] p-6 shadow-[0_12px_24px_-16px_rgba(85,158,123,0.12)]">
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          ...
          <p className="text-xs text-sage-dark/80 font-medium mt-0.5">Scope 3 — Cat 2: Capital Goods</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {method === 'supplier_specific' && (
          <div className="bg-surface border border-border rounded-xl p-4 space-y-3">
            ...
            <input
              required
              placeholder="Nama produk/aset kapital *"
              value={productName}
          ...
          <div>
            ...
            {avgFactors.length === 0 ? (
              <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
                <p className="text-xs text-amber-800">Belum ada faktor material Cat 2.</p>
              </div>
            ) : (
              <select
                required
        ...
        {method === 'spend_based' && (
          <div>
            ...
            {spendFactors.length === 0 ? (
              <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
                <p className="text-xs text-amber-800">Belum ada faktor EEIO Cat 2.</p>
              </div>
            ) : (
              <select
                required
