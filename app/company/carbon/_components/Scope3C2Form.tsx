...
import { submitS3C2 } from '@/lib/supabase/actions/carbon-activity'
...
const METHOD_INFO = {
  ...
  average_data: {
    ...
    hint: 'Faktor emisi rata-rata per jenis aset kapital (DEFRA/ecoinvent).',
    icon: (
...
export default function Scope3C2Form({ avgFactors, spendFactors }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (method === 'supplier_specific') {
      ...
      if (!safeUnit) { setError('Pilih atau ketik satuan supplier'); setLoading(false); return }
      result = await submitS3C2({ method, supplier_name: supplierName, product_name: productName, supplier_ef_kg_co2e_per_unit: parseFloat(supplierEf), quantity: q, unit: safeUnit, ...base })
    } else if (method === 'average_data') {
      if (!selectedAvg) { setError('Pilih material'); setLoading(false); return }
      result = await submitS3C2({ method, ef_average_id: selectedAvg.id, quantity: q, unit: selectedAvg.unit, ...base })
    } else {
      if (!selectedSpend) { setError('Pilih sektor'); setLoading(false); return }
      result = await submitS3C2({ method, ef_spend_id: selectedSpend.id, quantity: q, currency: selectedSpend.currency, ...base })
    }

    setLoading(false)
    if (result.error) { setError(result.error); toast.error(result.error) }
    else {
      toast.success('Draft Scope 3 Cat 2 berhasil disimpan')
    ...
    <div className="bg-surface border border-border rounded-[18px] p-6 shadow-[0_8px_16px_-12px_rgba(11,31,22,0.06)]">
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          ...
          <p className="text-sm font-extrabold text-ink tracking-tight">Cat 2: Capital Goods</p>
          <p className="text-xs text-muted mt-0.5">Emisi dari pembelian aset kapital</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {method === 'supplier_specific' && (
          <div className="bg-canvas/40 rounded-xl p-4 border border-border space-y-3">
            ...
            <input required placeholder="Nama produk/aset kapital *" value={productName} onChange={e => setProductName(e.target.value)}
              className="w-full px-4 py-2.5 bg-surface border border-border rounded-xl text-sm text-ink focus:outline-none focus:border-sage focus:ring-2 focus:ring-sage/20 transition-all" />
            <div className="grid grid-cols-2 gap-2">
              ...
              <div>
                ...
                <select value={supplierUnit === '__custom' ? '__custom' : supplierUnit}
                  onChange={e => { setSupplierUnit(e.target.value); if (e.target.value !== '__custom') setSupplierUnitCustom('') }}
                  className="w-full px-4 py-2.5 bg-surface border border-border rounded-xl text-sm text-ink focus:outline-none focus:border-sage focus:ring-2 focus:ring-sage/20 transition-all">
                  <option value="">Pilih satuan...</option>
                  {['kg', 'tonne', 'unit', 'm³', 'kWh', 'IDR', 'USD'].map(u => <option key={u} value={u}>{u}</option>)}
                  <option value="__custom">Lainnya...</option>
                </select>
                {supplierUnit === '__custom' && (
                  <input value={supplierUnitCustom} onChange={e => setSupplierUnitCustom(e.target.value)} placeholder="Ketik satuan..."
        ...
        {method === 'average_data' && (
          <div>
            <label className="block text-[10px] font-bold tracking-[0.14em] uppercase text-ink mb-1.5">
              Jenis Aset Kapital <span className="text-sage-dark">*</span>
            </label>
            {avgFactors.length === 0 ? (
              <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
                <p className="text-xs text-amber-800">Belum ada faktor material Cat 2. Admin perlu menambahkan.</p>
              </div>
            ) : (
        ...
        {method === 'spend_based' && (
          <div>
            ...
            {spendFactors.length === 0 ? (
              <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
                <p className="text-xs text-amber-800">Belum ada faktor EEIO Cat 2. Admin perlu menambahkan.</p>
              </div>
            ) : (
