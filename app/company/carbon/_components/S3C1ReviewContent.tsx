...
import EditS3C1Form from './EditS3C1Form'
...
export default function S3C1ReviewContent({ entry, avgFactors, spendFactors, methodLabel, itemLabel }: Props) {
  ...
  return (
    <>
      <ReviewPageLayout
        title={itemLabel}
        subtitle={`Scope 3 Cat 1 · ${methodLabel} · ${entry.quantity.toLocaleString('id-ID')} ${entry.unit}`}
        status={entry.status}
        id={entry.id}
        type="s3c1"
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            ...
            {entry.method === 'supplier_specific' && (
              <>
                ...
                <InfoCard label="Produk" value={entry.product_name ?? '—'} />
                <InfoCard label="EF Supplier" value={`${entry.supplier_ef_kg_co2e_per_unit ?? 0} kg CO₂e/${entry.unit}`} />
              </>
            )}
            {entry.method === 'average_data' && <InfoCard label="Material" value={entry.material_name ?? '—'} />}
