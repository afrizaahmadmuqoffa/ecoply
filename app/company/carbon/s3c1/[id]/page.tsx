...
import S3C1ReviewContent from '../../_components/S3C1ReviewContent'

export default async function S3C1ReviewPage({
  params,
}: {
  ...
  const [{ data: entry }, { data: avgFactors }, { data: spendFactors }] = await Promise.all([
    supabase.from('ca_s3c1').select('*').eq('id', id).eq('company_id', profile?.company_id ?? '').single(),
  ...
  return <S3C1ReviewContent entry={entry} avgFactors={avgFactors ?? []} spendFactors={spendFactors ?? []} methodLabel={methodLabel} itemLabel={itemLabel} />
}